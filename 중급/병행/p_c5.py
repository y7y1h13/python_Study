...
# concurrent.Futures
# 1. 멀티스레딩, 멀티 프로세싱 -> API 통일 -> 매우 사용하기 쉬움
# 2. 실행중인 작업 취소, 완료 여부 체크, 타임아웃 옵션, 콜백추가, 동기화 코드 매우 쉽게 작성 -> Promise 개념

# 2가지 패턴 실습
# concurrent.futures map 사용법
# concurrent.futures wait, as_completed 사용법

# GIL : 두 개 이상의 스레드가 동시에 실행 될 때 하나의 자원을 액세스 하는 경우 -> 문제점을 방지하기 위해 GIL 실행
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        ...
        # wait 결과 출력
        # result = wait(futures_list, timeout=2) # 시간내로 끝내지 못하면 실패로 간주하고 끝내버림
        # 성공
